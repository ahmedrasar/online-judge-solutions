...
void fillRegion(const int x, const int y, const char oc, const char nc)
{
    if (y == -1 || y == editor.size() || x == -1 || x == editor[0].size() || editor[y][x] != oc)
        return;
    editor[y][x] = nc;
    fillRegion(x - 1, y, oc, nc);
    fillRegion(x + 1, y, oc, nc);
    fillRegion(x, y - 1, oc, nc);
    fillRegion(x, y + 1, oc, nc);
}
